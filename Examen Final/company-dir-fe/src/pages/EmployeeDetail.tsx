...
export default function EmployeeDetail() {
  ...
  // TODO: Use useEffect to fetch the employee by id when the component mounts
    useEffect(() => {
      getEmployeeById(Number(id)).then((res) => setEmployees(res.data))
    }, [id])
  ...
  return (
    <main className="flex-1 w-full max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      className="inline-flex items-center gap-2 text-sm text-slate-500 hover:text-indigo-600 font-medium mb-8 transition-colors">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        ...
        <div className="px-6 sm:px-10 pb-10">
          ...
          <h2 className="text-2xl sm:text-3xl font-bold text-slate-900 mb-1">{employee.name}</h2>
          <p className="text-indigo-600 font-medium mb-8">{employee.position}</p>
